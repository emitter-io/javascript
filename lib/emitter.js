...
var Emitter = (function () {
     ...
     */
    Emitter.prototype.connect = function (request) {
        var _this = this;
        request = request || {};
        ...
        for (k in defaultConnectOptions) {
            this.request[k] = ('undefined' === typeof this.options[k])
                ? defaultConnectOptions[k]
                : request[k];
        }

        request.host = request.host.replace(/.*?:\/\//g, "");
        request.protocol = request.secure ? 'mqtts' : 'mqtt'; 
        
        var brokerUrl = request.protocol + '://' + request.host + ':' + request.port;
